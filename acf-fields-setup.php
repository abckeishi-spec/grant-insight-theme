...
// 助成金の締切日をフォーマットして取得
function gi_get_formatted_deadline($post_id = null) {
    $deadline_text = gi_get_field('deadline_text', $post_id);
    if (!empty($deadline_text)) {
        return $deadline_text;
    }
    
    $deadline_date = gi_get_field('deadline_date', $post_id);
    if (!empty($deadline_date)) {
        return date('Y年m月d日', strtotime($deadline_date));
    }
